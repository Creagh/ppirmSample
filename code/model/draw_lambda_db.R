...
# distribution
#
# INCLUDE MCMC SAMPLES FOR DELTA AND BETA
#########################################################

# params:
# delta - a vector of MCMC samples for delta
# beta - a vector of MCMC samples for beta

draw_lambda_db <- function(pi, edge_times, delta, beta, T) {
	...
	for(i in 1:num_samples) {
		...
		for(k in active_tables) {
			...
			for(l in active_tables) {
				...
				rate <- beta[i] +  ( T * m_counts[k] * length(table_l) )
				...
				shape <- n + delta[i] # the updated shape parameter
